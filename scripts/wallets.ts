...
import { checkDirectoriesInPath, gCurrentNetwork, ghre } from "./utils";
...
export const generateWalletBatch = async (
  ...
  let wallets: Promise<GasModelSigner>[] = [];
...
export const generateWallet = async (
  ...
  return new GasModelSigner(
    wallet.privateKey,
    ghre.lacchain.provider,
    gCurrentNetwork.nodeAddress,
    gCurrentNetwork.expiration
  );
};
...
export const decryptWallet = async (relativePath: string, password: string, connect?: boolean) => {
  ...
  return connect ? wallet.connect(ghre.lacchain.provider) : wallet;
};
